...
_default_log_file = (f"{sys.path[0]}\\logs\\{datetime.now().strftime('%Y-%m-%d')}.txt" if SystemUtils().get_system() == SystemUtils().windows else 
                     f"{sys.path[0]}/logs/{datetime.now().strftime('%Y-%m-%d')}.txt")
